...
SKIP=("try{var K='vkp.metabolic.v3';var r=localStorage.getItem(K);"
...
with sync_playwright() as pw:
    ...
    st=p.evaluate("""()=>{const s=JSON.parse(localStorage.getItem('vkp.metabolic.v3'));
    ...
    alpha=p.evaluate("""async()=>{const s=JSON.parse(localStorage.getItem('vkp.metabolic.v3'));
    ...
    check("cleared from storage", p.evaluate("()=>{const s=JSON.parse(localStorage.getItem('vkp.metabolic.v3'));return !(s.settings&&s.settings.signPhoto);}"))
